...
| Jeton d’accès   | Contenir les informations nécessaires pour accéder à une ressource Kaizala  | Le développeur utilise un jeton d’actualisation & d’autres détails du connecteur pour interroger le point de terminaison de l’API Kaizala pour générer un jeton d’accès    | 24 heures            |
...
Vous devrez utiliser le point de terminaison suivant pour obtenir un jeton d’accès (la première fois & ultérieurement lorsque le jeton d’accès expire):
