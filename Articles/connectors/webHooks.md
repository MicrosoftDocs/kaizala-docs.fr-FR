...
---
# <a name="apis-to-register--manage-webhooks"></a>API pour enregistrer & gérer les webhooks
...
Pour vérifier que votre point de terminaison de service webhook est authentique et que nous travaillons, nous allons vérifier votre URL de rappel avant de créer un abonnement. Pour la vérification, nous vous enverrons un jeton de validation dont vous avez besoin pour nous envoyer vos commentaires dans un délai de 5 secondes. [En savoir plus](WebHookValidation.md)

#### <a name="request-parameters"></a>Paramètres de la demande

|  | Paramètre | Type | Module? | Description |
| :---: | :---: | :---: | :---: | :--- |
| En-tête HTTP | accessToken | Chaîne | Non | Jeton d’accès reçu depuis le point de terminaison auth || En-tête HTTP | Content-Type | Chaîne | Non | «application/JSON» |

#### <a name="request-body"></a>Corps de la demande

|  Paramètre | Type | Module? | Description |
| :---: | :---: | :---: | :--- |
| objectId | Chaîne | Non | Identificateur représentant l’objet dans lequel le contexte des webhooks doit être créé. Pour ObjectType = Group, l’identificateur de son groupe, pour ObjectType = action, son actionId, pour ObjectType = ActionPackage, son action-Package-ID |
| objectType | Chaîne | Non | Énumération: «Group»/«action»/«ActionPackage» |
...
| callBackUrl | Chaîne | Non | URL HTTPs à laquelle les événements abonnés doivent être notifiés |
...
| Paramètre de requête | objectId | Chaîne | Non | Identificateur représentant l’objet dans lequel le contexte des webhooks doit être créé. Pour ObjectType = Group, l’identificateur de son groupe, pour ObjectType = action, son actionId, pour ObjectType = ActionPackage, son action-Package-ID |
| Paramètre de requête | objectType | Chaîne | Non | Énumération: «Group»/«action»/«ActionPackage» |
...
| Paramètre | Type | Description |
| :---: | :---: | :--- |
| id | Chaîne | Identificateur de webhook |
...
| :---: | :---: | :---: | :---: | :--- |
| En-tête HTTP | accessToken | Chaîne | Non | Jeton d’accès reçu depuis le point de terminaison auth |
| Paramètre Path | webhookId | Chaîne | Non | Identificateur de webhook |
