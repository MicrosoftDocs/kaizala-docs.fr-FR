...
Supprimer la clé donnée dans le cache de données local

**Paramètres :**

| Nom | Type | Description |
| ------ | ------ | ------ |
| actionLocalCacheProps | [KASActionLocalCacheProp](../classes/kasclient.kasactionlocalcacheprop.md) |  propriété de données à supprimer du cache |
| callback | `function` |  rappel avec les paramètres suivants \* \* : @param {Boolean} Success indique si la mise à jour a \* \* réussi ou non @param chaîne JSON de l’erreur {String} pour l’objet KASError contenant un code d’erreur et/ou une description. |
...
Supprime un fichier du stockage de cache temporaire. Utilisé conjointement avec l’API saveDataInTmpDirAsync pour les fichiers stockés à l’aide de cette API.
...
| Nom | Type | Description |
| ------ | ------ | ------ |
| PCF | `string` |  FilePath qui doit être lu |
| callback | `function` |  avec les paramètres suivants: |
...
Extrait les détails de l’attribut client. Le client de la conversation dans le contexte sera utilisé pour cette opération.
#### <a name="note"></a>Remarque

L’action doit appartenir au même client de la conversation et l’utilisateur doit être connecté à ce client pour que cette API fonctionne.
...
| Nom | Type | Description |
| ------ | ------ | ------ |
| callback | `function` |  avec les paramètres suivants:<br><br>\*@param {TenantAttribute\[\]} tenantAttributes tableau d’attributs client<br><br>\*message d’erreur @param {String} en cas d’erreur, NULL sinon |
...
Extrait les attributs de client des utilisateurs spécifiés. Le client de la conversation dans le contexte sera utilisé pour cette opération.
#### <a name="note"></a>Remarque
...
| Nom | Type | Description |
| ------ | ------ | ------ |
| userIds | `string`[] |  Tableau des ID d’utilisateur; s’il est null ou vide, le profil client de l’utilisateur actuel sera extrait. |
| callback | `function` |  avec les paramètres suivants:<br><br>\*@param {TenantUserProfile\[\]} tenantUserProfiles tableau des profils client des utilisateurs (paires ID-valeur d’attribut)<br><br>\*message d’erreur @param {String} en cas d’erreur, NULL sinon |
...
Récupère la clé donnée à la valeur du cache de données local est enregistré au niveau indiqué dans KASActionLocalCacheProp en conséquence.

**Paramètres :**

| Nom | Type | Description |
| ------ | ------ | ------ |
| actionLocalCacheProps | [KASActionLocalCacheProp](../classes/kasclient.kasactionlocalcacheprop.md) |  propriété de données à récupérer du cache |
| callback | `function` |  rappel avec les paramètres suivants \* \* : @param {KASActionLocalCacheProp} actionLocalCacheProps indique si la mise à jour a \* \* réussi ou non @param chaîne JSON de l’erreur {String} pour l’objet KASError contenant le code d’erreur et/ ou description. |
...
| `Default value`canUseCachedLocation | `boolean` | false |  (facultatif, valeur par défaut si la valeur est false) si cet indicateur a la valeur true, la plateforme peut choisir de renvoyer un emplacement mis en cache de jusqu’à 30min ancienne en cas d’erreur lors de l’extraction de l’emplacement actuel |
...
Lit le contenu du fichier en base64 à partir du stockage de cache temporaire. Utilisé conjointement avec l’API saveDataInAppCacheAsync pour les fichiers stockés à l’aide de cette API.
...
Enregistre les données base64 sur l’appareil avec le nom de fichier donné. Les actions peuvent utiliser cette API pour stocker des données temporairement sur l’appareil de stockage, qui peuvent être référencées dans les charges utiles de mise à jour de formulaire/réponse/propriétés au cours de cette session. Notez que ces données sont stockées dans le répertoire de cache temporaire local et peuvent être supprimées par le système d’exploitation de l’appareil sans avertissement dans les scénarios de stockage faible. La durée de vie maximale de ce stockage est comprise dans une session de l’action. Une fois que l’écran se ferme, ces données sont désactivées. En règle générale, l’action peut utiliser ce stockage pour enregistrer des données d’image/audio base64 sur le stockage et faire référence à ce chemin dans Survey JSON/Response and client s’assurera qu’il est téléchargé vers le service dans le flux d’envoi de messages.
...
| Nom | Type | Description |
| ------ | ------ | ------ |
| base64Data | `string` |  données base64 à stocker. |
| fileName | `string` |  Nom de fichier, extension comprise, qui doit être utilisé pour stocker les données. Le nom de fichier longueur maximale autorisée est 15 et ne peut contenir que des caractères alphanumériques, des traits de soulignement, des hifen et des points,\_par exemple «a-zA-z0-9.-». Par exemple, fichier1. mp3 |
| callback | `function` |  avec les paramètres suivants: |
...
▸ **showImageImmersiveView**(URL?: * `string`[]*, currentImageIndex?: *`number`*):`void`
...
Updates/enregistre la valeur donnée par rapport à la clé dans le cache de données local

**Paramètres :**

| Nom | Type | Description |
| ------ | ------ | ------ |
| actionLocalCacheProps | [KASActionLocalCacheProp](../classes/kasclient.kasactionlocalcacheprop.md) |  propriété de données à enregistrer dans le cache |
| callback | `function` |  rappel avec les paramètres suivants \* \* : @param {Boolean} Success indique si la mise à jour a \* \* réussi ou non @param chaîne JSON de l’erreur {String} pour l’objet KASError contenant un code d’erreur et/ou une description. |
...
Met à jour les attributs du client de l’utilisateur actuel. Le client de la conversation dans le contexte sera utilisé pour cette opération.
#### <a name="note"></a>Remarque
...
| Nom | Type | Description |
| ------ | ------ | ------ |
| attributeDataList | [TenantAttributeData](../classes/kasclient.tenantattributedata.md) [] |  paires ID-valeur d’attribut client |
| callback | `function` |  avec les paramètres suivants:<br><br>\*@param {Boolean} Success true si elle réussit, false dans le cas contraire<br><br>\*message d’erreur @param {String} en cas d’erreur, NULL sinon |
